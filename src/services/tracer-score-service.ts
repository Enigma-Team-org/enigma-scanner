...
function calculateTrust(data: AgentData): DimensionScore {
  ...
  // Validaciones recibidas (simulado por ahora)
  const validationsScore = Math.min(data.daysSinceRegistration * 0.5, 40);
...
function calculateReliability(data: AgentData): DimensionScore {
  ...
  return {
    ...
    components: {
      ...
      latency: data.avgResponseTimeMs < 100 ? 30 : data.avgResponseTimeMs < 300 ? 25 : 20,
...
function calculateAutonomy(data: AgentData): DimensionScore {
  ...
  // Tasa de no intervención (simulado)
  const noInterventionRate = data.canDelegate ? 70 : 40;
...
function calculateCapability(data: AgentData): DimensionScore {
  ...
  // Outputs auditados (simulado)
  score += Math.min(data.txCount * 0.1, 35);

  // Certificaciones (simulado)
  score += data.hasAudits ? 20 : 0;
  ...
  return {
    ...
    components: {
      skillsVerified: data.skillsDeclared.length > 0 
        ...
        : 10,
      auditedOutputs: Math.min(Math.round(data.txCount * 0.1), 35),
      certifications: data.hasAudits ? 20 : 0,
    },
  };
}
...
function calculateEconomics(data: AgentData): DimensionScore {
  ...
  // Previsibilidad (simulado)
  score += 35;

  // Eficiencia de gas (simulado)
  score += data.isProxy ? 20 : 30;
  ...
  return {
    ...
    components: {
      predictability: 35,
      gasEfficiency: data.isProxy ? 20 : 30,
      paymentHistory: 25,
    },
  };
}
...
function calculateReputation(data: AgentData): DimensionScore {
  ...
  // Conexiones de confianza (simulado)
  score += Math.min(data.skillsVerified.length * 5, 30);

  // Endosos (simulado)
  score += data.hasAudits ? 20 : 10;
  ...
  return {
    ...
    components: {
      feedback: data.ratings.length > 0 
        ...
        : 20,
      trustConnections: Math.min(data.skillsVerified.length * 5, 30),
      endorsements: data.hasAudits ? 20 : 10,
    },
  };
}
